...
species <- variables[5, 1]

#Reading in fasta of interest
peps <- seqinr::read.fasta(file=paste0(variables[2, 1], species, "/Transdecoder/", "Trinity.Trinity.fasta.transdecoder.pep", sep=""), seqtype = "AA", as.string = T, whole.header = T)
...
rename.fasta(infile = paste0(variables[2, 1], species, "/Transdecoder/", "Trinity.Trinity.fasta.transdecoder.pep", sep=""), ref_table = pepnames, outfile = paste0(variables[2, 1], species, "/Transdecoder/", "Trinity.Trinity.fasta.transdecoder.rename.fa", sep=""))



###Okay, here we're making the splice file
#Reading in the gene_trans_map of interest
gene_trans_map <- read.delim(paste0(variables[2, 1], species, "/Trinity.Trinity.fasta.gene_trans_map", sep=""), header=F, sep="\t")
...
peps_rename <- seqinr::read.fasta(file=paste0(variables[2, 1], species, "/Transdecoder/", "Trinity.Trinity.fasta.transdecoder.rename.fa", sep=""), seqtype = "AA", as.string = T, whole.header = T)
...
write.table(gorl$trans, file=paste0(variables[2, 1], species, "/Transdecoder/", "Trinity.Trinity.fasta.transdecoder.splice", sep=""), sep="", row.names=F, col.names=F, quote=F)